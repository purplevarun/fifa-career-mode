...
def classify(text):
    ...
    if "standings" in normalized:
        if re.search(r"(?:grabs|wins)(?:" + "|".join(month.lower() for month in MONTHS) + r")playerofthemonth", normalized):
...
def parse_money(text):
    matched = re.fullmatch(
        ...
        r"((?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d{1,2})?)\s*([KM])?\s*", text, re.IGNORECASE,
    )
    if not matched:
    ...
    currency = {"\u00a3": "GBP", "\u20ac": "EUR", "\u20b9": "INR", "RS": "INR"}.get(
        (symbol or "").upper(), symbol.upper() if symbol and symbol != "$" else None,
    )
    return amount, currency, symbol


def parse_months(text):
    matched = re.fullmatch(r"\s*(\d+)\s*-?\s*(year|month)s?(?:\s*[\[(]s[\])])?\s*", text, re.IGNORECASE)
...
def detected_fields(tokens, regions):
    ...
    for field, bounds in regions.items():
        ...
        selected = [token for token in tokens
                    if normalized[0] <= sum(point[0] for point in token["box"]) / len(token["box"]) <= normalized[2]
                    and normalized[1] <= sum(point[1] for point in token["box"]) / len(token["box"]) <= normalized[3]]
        selected.sort(key=lambda token: (min(point[1] for point in token["box"]), min(point[0] for point in token["box"])))
        fields[field] = {"box": normalized, "method": "rapidocr_detected_region",
...
def parse_news_date(text):
    ...
    if matched:
        try:
            return date(int(matched.group(3)), int(matched.group(2)), int(matched.group(1))).isoformat()
        except ValueError:
            pass
    return None
...
def parse_calendar_month(text):
    ...
    matched = re.search(r"(?<![a-z])(" + labels + r")\s*(?:(\d{1,2})\s*,\s*)?(20\d{2})\b", text, re.IGNORECASE)
    if matched:
        month = next(index + 1 for index, name in enumerate(MONTHS) if name[:3].casefold() == matched.group(1)[:3].casefold())
        try:
            return date(int(matched.group(3)), month, int(matched.group(2) or 1)).strftime("%Y-%m")
        except ValueError:
            pass
    return None
...
def parse_news_event(title, body="", announced_on=None, observed_on=None):
    ...
    if any(word in normalized for word in ("shortlist", "nominee", "nominated", "nomination", "expected", "likely", "prediction")):
        return None
    player = club = period = event_type = None
    competition = parse_header(f"{title} {body}")["competition"]
    event_date = date.fromisoformat(announced_on or observed_on) if announced_on or observed_on else None
    monthly = re.match(
        r"(.+?)\s*(?:grabs|wins)\s*(" + "|".join(MONTHS) + r")\s*Player\s*of\s*the\s*Month",
        title, re.IGNORECASE,
    )
    champion = re.fullmatch(r"(.+?)\s*Crowned\s*(.+?)\s*Champions?[.!]?", title, re.IGNORECASE)
    golden_boot = re.fullmatch(r"(.+?)\s*Wins\s*Golden\s*Boot[.!]?", title, re.IGNORECASE)
    award = re.fullmatch(r"(player|goalkeeper)ofthe(competition|tournament)(?:announced)?", normalized)
    if monthly:
        event_type = "player_of_the_month"
        player = monthly.group(1).strip()
        month = next(index + 1 for index, name in enumerate(MONTHS) if name.casefold() == monthly.group(2).casefold())
        ...
        ownership = re.match(r"(.+?)['\u2019]s\s*(.+?)\s*has\s*been\s*named", body, re.IGNORECASE)
    ...
    return {
        "type": "competition_event", "event_type": event_type, "player": player, "club": club,
        "competition": competition, "season": season,
        "announced_on": announced_on, "period": period or season, "description": f"{title}. {body}".strip(),
    }
...
class ScreenshotExtractor:
    ...
    def extract(self, path):
        ...
        if screen_type == "match_facts":
            ...
            issues.append("Confirm the competition season and announcement date; the winner screen does not show them.")
        else:
            return {"screen_type": screen_type, "records": [], "evidence": evidence,
        ...
        return {"screen_type": screen_type, "records": records, "evidence": evidence, "issues": issues}

    def extract_transfer(self, tokens):
        fields = detected_fields(tokens, {
            "panel_heading": (684, 195, 1040, 225), "player": (697, 228, 986, 287),
            "to_club": (733, 287, 982, 309), "weekly_wage_minor": (692, 357, 806, 383),
            "contract_months": (813, 357, 928, 383), "loan_months": (733, 508, 829, 565),
            "transfer_type": (953, 508, 1053, 565), "completion": (685, 385, 1054, 453),
        ...
        transfer_type = {"transfer": "permanent", "loan": "loan"}.get(compact(text["transfer_type"]))
        if (compact(text["panel_heading"]) != "contractoffers" or not completed
                or compact(text["status"]) != "accepted" or not transfer_type
                or not text["player"] or not text["to_club"]):
            return [], fields, ["No completed transfer or loan confirmed in the selected contract panel; offers are not signings."]
        date_text = ""
        surname = compact(text["player"].split()[-1])
        selected_rows = [token for token in tokens
                         if 320 / BASE_WIDTH < min(point[0] for point in token["box"]) < 520 / BASE_WIDTH
                         and 255 / BASE_HEIGHT < min(point[1] for point in token["box"]) < 645 / BASE_HEIGHT
                         and compact(token["text"]).endswith(surname)]
        if len(selected_rows) == 1:
            center = sum(point[1] for point in selected_rows[0]["box"]) / len(selected_rows[0]["box"]) * BASE_HEIGHT
            fields.update(detected_fields(tokens, {"effective_on": (580, center - 14, 667, center + 14)}))
            date_text = fields["effective_on"]["raw_text"]
        wage, currency, symbol = parse_money(text["weekly_wage_minor"])
        record = {
            "type": "player_transfer", "player": text["player"], "from_club": None,
            "to_club": club_name(text["to_club"]), "transfer_type": transfer_type,
            "effective_on": None, "date_basis": f"{date_text or 'No exact date'} shown in the selected transfer row; year requires review.",
            "fee_minor": None, "weekly_wage_minor": wage, "currency": currency, "currency_display": symbol,
            "contract_months": parse_months(text["contract_months"]),
            "loan_months": parse_months(text["loan_months"]) if transfer_type == "loan" else None,
        }
        return [record], fields, ["Confirm the transfer date/year and selling club; neither is inferred from a badge or filename.",
    ...
    def extract_news(self, tokens):
        fields = detected_fields(tokens, {"article.title": (949, 225, 1267, 290), "article.body": (949, 300, 1267, 404)})
        ...
        for row_index in range(6):
            ...
            fields.update(detected_fields(tokens, {
        ...
        matching = [announced_on for headline, announced_on in stories if compact(news_title(headline)) == compact(news_title(title))]
        selected = parse_news_event(title, body, matching[0] if len(matching) == 1 else None)
        ...
        issues = []
        if not records:
            issues.append("No named award winner or championship found; predictions and shortlists are not awards.")
        for record in records:
            if not record["competition"] or not record["season"]:
                issues.append(f"Confirm competition/season for {record['description']}; unrelated articles are not used as context.")
            if record["player"]:
                issues.append(f"Confirm the full player identity for {record['player']}; news may display only a surname.")
        return records, fields, issues

    def extract_competition_result(self, tokens):
        fields = detected_fields(tokens, {
            "competition": (139, 74, 654, 105), "club": (300, 509, 655, 550),
        ...
        if "winner" not in confirmation or "congratulations" not in confirmation or not winner:
            return [], fields
        return [{"type": "competition_event", "event_type": "champion", "player": None, "club": winner,
                 "competition": competition, "season": None, "announced_on": None, "period": None,
                 "description": f"{winner} won {fields['competition']['raw_text']}."}], fields

    def extract_dashboard_award(self, image, tokens):
        fields = detected_fields(tokens, {
            "calendar": (590, 208, 676, 236), "fixture_date": (87, 248, 373, 270),
        ...
        months = {parse_calendar_month(fields[field]["raw_text"]) for field in ("calendar", "fixture_date")}
        months.discard(None)
        observed_on = f"{next(iter(months))}-01" if len(months) == 1 else None
        record = parse_news_event(fields["headline"]["raw_text"], observed_on=observed_on)
        if record is None:
            record = parse_news_event(fields["headline.detected"]["raw_text"], observed_on=observed_on)
        return ([record] if record else []), fields, [
