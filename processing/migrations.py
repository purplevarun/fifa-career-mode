...
def migrate_to_uuids(connection, database_path, schema):
    ...
    backup_path = (
        ...
        / "backups"
        / f"{database_path.stem}-before-uuid-{timestamp}.sqlite"
    )
    ...
    mappings = {
        ...
        for table in tables
        if "id" in old_columns[table]
    }
    drops = [
        f'DROP {row[0].upper()} "{row[1]}";'
        for row in connection.execute(
    ...
    try:
        ...
        for table, identities in mappings.items():
            connection.executemany(
                "INSERT INTO legacy_ids(entity_table, old_value, uuid) VALUES (?, ?, ?)",
                (
                    (table, old_value, identifier)
                    for old_value, identifier in identities.items()
                ),
            )
        revisions = {}
        seen_hashes = set()
        for table in tables:
            ...
            for row in connection.execute(
                f'SELECT * FROM "legacy_{table}" ORDER BY {ordering}'
            ).fetchall():
                values = dict(row)
                ...
                if (
                    ...
                    and "goals_conceded_displayed" not in values
                ):
                ...
                if table == "extractions":
                    values["candidate_json"] = json.dumps(
                        normalize_references(
                            connection,
                            json.loads(values["candidate_json"]),
                            allow_legacy=True,
                        ),
                        ensure_ascii=True,
                    )
                if table == "player_snapshots" and values.get(
                    "date_basis", ""
                ).startswith("Performance screenshot linked to approved match "):
                    values["date_basis"] = (
                        f"Performance screenshot linked to approved match {values['match_id']}"
                    )
                if table == "reviews":
                    ...
                    payload = normalize_references(
                        connection,
                        json.loads(values["payload_json"]),
                        allow_legacy=True,
                    )
                    ...
                    if (source_id, payload_hash) in seen_hashes:
                        payload_hash = hashlib.sha256(
                            f"{serialized}:migrated_revision:{revision}".encode()
                        ).hexdigest()
                    values["payload_hash"] = payload_hash
                    seen_hashes.add((source_id, payload_hash))
                values = {
                    field: value for field, value in values.items() if field in columns
                }
            ...
            old_count = connection.execute(
                f'SELECT COUNT(*) FROM "legacy_{table}"'
            ).fetchone()[0]
            ...
            if old_count != new_count:
                raise ValueError(
                    f"Migration changed {table} record count: {old_count} -> {new_count}"
                )
