  ...
  const columns: Column<Match>[] = [
    {
      ...
      render: (match) => (
        <span className="nowrap">
          ...
          <small className="muted">{match.played_on.slice(0, 4)}</small>
        </span>
      ),
    },
    {
      ...
      title: "Fixture",
      render: (match) => (
        <CareerLink to={`/matches/${match.id}`} className="fixture-link">
    ...
    },
    {
      ...
      render: (match) => {
        ...
        return source ? <SourceButton sourceId={source.source_id} /> : null;
      },
    },
  ];
  if (compact)
...
export function Overview() {
  ...
  const leaders = model.data.players
    ...
    .sort((left, right) => (right.goals.value ?? 0) - (left.goals.value ?? 0))
    .slice(0, 6);
  ...
  return (
    <>
      <PageHeading title="Career overview" eyebrow="Notts County / FIFA 19">
            ...
            <section>
              <SectionHeading title="Goals across the career">
                <span className="tiny muted">
                  {matches[0]?.played_on} to {latest?.played_on}
                </span>
              </SectionHeading>
              <TrendChart
                ...
                series={[
                  { key: "scored", name: "Scored", color: "#247451" },
                  { key: "conceded", name: "Conceded", color: "#c2754c" },
                ]}
              />
            </section>
            <section>
              <SectionHeading title="Leading scorers">
                <CareerLink to="/players" className="small-link">
          ...
          <div className="overview-lower">
            <section>
              <SectionHeading title="Recent fixtures">
                <CareerLink to="/matches" className="small-link">
            ...
            <section className="latest-evidence">
              <SectionHeading title="Latest match">
                <span className="tiny muted">{latest?.competition.name}</span>
              </SectionHeading>
              {source && (
                <button
              ...
              {latest && (
                <div className="latest-caption">
                  <CareerLink to={`/matches/${latest.id}`}>
                    <strong>
                      {latest.home} {display(latest.home_goals)}-
                      {display(latest.away_goals)} {latest.away}
                    </strong>
                  </CareerLink>
                  <span>
                    {dateLabel(latest.played_on)} / {latest.venue}
                  </span>
                </div>
              )}
            </section>
      ...
      <div className="archive-note">
        ...
        <strong>
          {model.data.match_coverage.summary.matches as number} fixtures
          verified
        </strong>
        <span>{display(model.data.player_matches.length)} player records</span>
        <CareerLink to="/audit">
          {model.data.match_coverage.warnings.length} reconciliation warning{" "}
          <ArrowRight size={14} />
        </CareerLink>
      </div>
    </>
...
export function PerformanceDialog({
  ...
  return (
    <Modal
      title={
        model.players.get(performance.player_id)?.name ?? "Player performance"
      }
      onClose={onClose}
      wide
    >
      <div className="performance-summary">
        <div>
          <Pill>{performance.displayed_position ?? "Position unknown"}</Pill>
          <h3>
            {match?.home} {display(match?.home_goals)}-
            {display(match?.away_goals)} {match?.away}
          </h3>
          <p className="muted">
            {match && dateLabel(match.played_on)} / {match?.competition.name}
          </p>
        </div>
              ...
              <dl className="stats-list">
                {fields.map((field) => (
                  <div key={field}>
                    ...
                    >
                      {display(performance[field], field === "rating" ? 1 : 0)}
                    </dd>
                  </div>
                ))}
              </dl>
...
export function MatchDetail() {
  ...
  const homeStats = model.data.team_matches.find(
    (row) => row.match_id === match.id && row.club_id === match.home_club_id,
  );
  const awayStats = model.data.team_matches.find(
    (row) => row.match_id === match.id && row.club_id === match.away_club_id,
  );
  ...
  const columns: Column<Performance>[] = [
    ...
    { key: "overall", title: "OVR", numeric: true },
    {
      ...
      numeric: true,
      render: (row) => (
        <span className={`rating ${Number(row.rating) >= 8 ? "high" : ""}`}>
    ...
    { key: "goals_conceded", title: "GK conceded", numeric: true },
    {
      ...
      sortable: false,
      render: (row) => (
        <CareerLink to={`/players/${row.player_id}`} className="small-link">
    ...
    <>
      ...
      {warnings.map((warning, index) => (
        <div className="notice warning" key={index}>
          ...
          <span>
            Team score: {String(warning.team_goals)}. Credited player goals:{" "}
            {String(warning.credited_player_goals)}. Attribution remains
            unconfirmed.
          </span>
        </div>
      ))}
...
export function Players() {
  ...
  const players = model.data.players
    .map((player) =>
      playerSummary(model, player, matches, opposition ? null : model.nottsId),
    )
    .filter(
      (player) =>
  ...
  const columns: Column<PlayerSummary>[] = [
    {
      ...
      title: "Player",
      render: (player) => (
        <CareerLink to={`/players/${player.id}`} className="player-cell">
          ...
          <span>
            ...
            <small>{player.nationality ?? "Nationality not recorded"}</small>
          </span>
        </CareerLink>
      ),
    },
      ...
      <PageHeading title="Players" eyebrow="Performance & development">
        <label className="checkbox">
          <input
            type="checkbox"
            checked={opposition}
            onChange={(event) => setOpposition(event.target.checked)}
          />
          Include opposition
        </label>
      </PageHeading>
...
export function PlayerDetail() {
    ...
    (row) =>
      ...
      (filters.competition === "all" ||
        (typeof row.competition_season_id === "string" &&
          model.editions.get(row.competition_season_id)?.competition_id ===
            filters.competition)),
  );
  const occurrenceColumns: Column<Performance>[] = [
      ...
      render: (row) => {
        ...
        return (
          <span className="stack">
            <strong>
              {dateLabel(match.played_on, true)} / {match.opponent}
            </strong>
            <small className="muted">{match.competition.name}</small>
          </span>
        );
      },
    },
    ...
    { key: "assists", title: "Assists", numeric: true },
    {
      ...
      sortable: false,
      render: (row) => (
        <CareerLink to={`/matches/${row.match_id}`} className="small-link">
      ...
      {tab === "appearances" && (
        <>
          ...
          <TrendChart
            ...
            series={[{ key: "rating", name: "Match rating", color: "#247451" }]}
            domain={[0, 10]}
            height={205}
          />
          <DataTable
      ...
      {tab === "development" && (
        <>
          <SectionHeading title="Recorded overall history">
            <span className="tiny muted">{snapshots.length} observations</span>
          </SectionHeading>
          <TrendChart
            rows={snapshots
            ...
            series={[{ key: "overall", name: "OVR", color: "#247451" }]}
                ...
                render: (row) => (
                  <span className="stack">
                    {row.observed_on
                      ...
                      : model.seasons.get(row.season_id)?.label}
          ...
          <DataTable
            ...
            columns={[
                    ...
                    <strong>
                      {row.scope === "all_competitions"
                        ...
                        : model.competitions.get(
                            model.editions.get(
                              String(row.competition_season_id),
                            )?.competition_id ?? "",
                          )?.name}
                    </strong>
                    <small className="muted">
                      {model.seasons.get(String(row.season_id))?.label} /{" "}
              ...
              },
              {
                ...
                title: "Evidence",
                render: (row) => (
                  <SourceButton sourceId={String(row.source_id)} />
                ),
              },
            ]}
          />
  ...
  return (
    <>
      <PageHeading title="Competitions" eyebrow="League & cup records">
        <span className="muted">{competitions.length} competitions</span>
      </PageHeading>
            ...
            >
              ...
              <div className="card-foot">
                <span>
                  ...
                  {display(summary.goalsAgainst.value)} conceded
                </span>
                {competition.kind === "league" && (
