...
export function StatValue({
  ...
  const partial =
    known !== undefined && total !== undefined && known > 0 && known < total;
  return (
    <span
      className="number"
      title={
                  ...
                  >
                    {onSelect && index === 0 ? (
                      ...
                      >
                        {column.render
                          ? column.render(row)
                          : display(column.value?.(row) ?? row[column.key])}
                      </button>
                    ) : column.render ? (
                      column.render(row)
                    ) : (
  ...
  return (
    ...
    >
      <div className="source-toolbar">
        ...
        <Pill tone={source.status === "imported" ? "positive" : "warning"}>
      ...
      {Object.entries(record).map(([field, value]) => (
        <div key={field}>
          ...
          <dd>
            ...
            ) : field === "source_id" && typeof value === "string" ? (
              <SourceButton sourceId={value} text />
            ) : (
              <>
                <span
                  className={
                    field === "id" || field.endsWith("_id") ? "code-value" : ""
                  }
                >
                  {referenceLabel(model, field, value)}
                </span>
                {field.endsWith("_id") && value && (
                  <small className="code-value muted">{String(value)}</small>
                )}
              </>
            )}
          </dd>
